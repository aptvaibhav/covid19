  ...
  render() {
    return (
      <div className="App">
        ...
        <p className="description">COVID19 Cases in India Till Now</p>
              ...
              <div className="card-content white-text">
                ...
                {this.state.covid && this.state.indiaLoaded ? (
                  <span className="cardData">
                    {parseInt(this.state.covid.statewise[0].confirmed) + 15}
                  </span>
                ) : (
                  <span className="cardData">Loading..</span>
                )}
              </div>
            </div>
          </div>
              ...
              <div className="card-content white-text">
                ...
                {this.state.covid && this.state.indiaLoaded ? (
                  <span className="stat-ci">
                    {parseInt(this.state.covid.statewise[0].active) + 15}
                  </span>
                ) : (
                  <span className="cardData">Loading..</span>
                )}
          ...
          <div className="col s6 m6">
            <div className="card-block card blue-grey">
              <div className="card-content white-text">
                ...
                {this.state.covid && this.state.indiaLoaded ? (
                  <span className="stat-re">
                    {this.state.covid.statewise[0].recovered}
                  </span>
                ) : (
                  <span className="cardData">Loading..</span>
                )}
          ...
          <div className="col s6 m6">
            <div className="card-block card blue-grey">
              <div className="card-content white-text">
                ...
                {this.state.covid && this.state.indiaLoaded ? (
                  <span className="stat-de">
                    {this.state.covid.statewise[0].deaths}
                  </span>
                ) : (
                  <span className="cardData">Loading..</span>
                )}
          ...
          <div className="col s12 m12">
            {this.state.covid && this.state.indiaLoaded ? (
              <p className="lastUpdated">
                Last Updated :{" "}
                {moment(
                  this.state.covid.statewise[0].lastupdatedtime,
                  "DD/MM/YYYY hh:mm:ss"
        ...
        <div className="row">
          ...
          <div className="col s12 m12">
            {this.state.showWorld ? (
              <select
                className="browser-default chooseOption chooseOpt"
                onChange={this.handleChange}
              >
                <option value="">Choose your country</option>
                {countries.map((data, idx) => {
                  return (
                    <option value={data.code} key={idx} name={data.name}>
                      {" "}
          ...
          <div className="col s12 m12">
            ...
            this.state.otherCovid.confirmed &&
            this.state.otherCovid.recovered &&
            this.state.otherCovid.deaths &&
            this.state.otherCovid.lastUpdate ? (
              <div className="stateBox">
                <p>
                  Total in {this.state.otherCovidName}:{" "}
                  {this.state.otherCovid.confirmed.value}
                </p>
                <p>Recoverd : {this.state.otherCovid.recovered.value}</p>
                <p>Death : {this.state.otherCovid.deaths.value}</p>
                <p className="lastUpdated">
                  Last Updated :{" "}
                  {moment(this.state.otherCovid.lastUpdate).format(
                    "MMMM Do YYYY, h:mm:ss A"
                  )}
                </p>
              </div>
            ) : this.state.loadingOther ? (
              <p className="loadingOtherColor">Loading...</p>
            ) : null}
            {this.state.noOtherCovid ? (
              this.state.showWorld ? (
                <p className="loadingOtherColor">No Patient Here, Stay Safe</p>
              ) : null
            ) : null}
          </div>
